...
const unbracketAddress = (host: string): string => (typeof host === 'string' && host.startsWith('[') && host.endsWith(']') ? host.slice(1, -1) : host);
...
const formatAuthority = (host: string, port: number): string => {
...
const redactUrl = (proxyUrl: URL): string => {
...
const stripProxyCredentials = <T>(err: T): T => {
    if (err && typeof err === 'object') {
        delete (err as { options?: unknown }).options;
        delete (err as { input?: unknown }).input;
    }
    return err;
};
...
const attachEarlyErrorHandler = (logger: InternalLogger, socket: ProxySocket | null | undefined): void => {
    if (!socket || typeof socket.on !== 'function') {
        return;
    }
    socket._earlyErrorHandler = (err: Error) => {
...
const detachEarlyErrorHandler = (socket: ProxySocket | false | null | undefined): void => {
...
interface HttpConnectParams {
    ...
    /** Parsed proxy URL (credentials intact) */
    proxyUrl: URL;
    /** Whether the proxy endpoint itself speaks TLS */
    secureProxy: boolean;
    /** Proxy endpoint host, IPv6 literals unbracketed */
    proxyHost: string;
    /** Proxy endpoint port */
    proxyPort: number;
 ...
 */
const httpConnect = async ({ logger, proxyUrl, secureProxy, proxyHost, proxyPort, host, port, deadline }: HttpConnectParams): Promise<ProxySocket> => {
    ...
    let socket: ProxySocket | null = null;
    ...
    return await new Promise<ProxySocket>((resolve, reject) => {
        ...
        let connectOptions: net.NetConnectOpts & tls.ConnectionOptions = { host: proxyHost, port: proxyPort };
        if (secureProxy) {
...
interface SocksConnectParams {
    ...
    /** Parsed proxy URL (credentials intact) */
    proxyUrl: URL;
    /** Configured proxy protocol (socks, socks4, socks4a, socks5) */
    protocol: string;
    /** Proxy endpoint host, IPv6 literals unbracketed */
    proxyHost: string;
    /** Proxy endpoint port */
 ...
 * @returns The established socket, tunnelled to the destination.
 */
const socksConnect = async ({ logger, proxyUrl, protocol, proxyHost, proxyPort, host, port, deadline }: SocksConnectParams): Promise<ProxySocket> => {
    let proxyType: SocksProxy['type'] = protocol === 'socks4' || protocol === 'socks4a' ? 4 : 5;
    ...
    try {
        ...
        let connectionOpts: SocksClientOptions = {
            proxy: {
        ...
        let err = deadline.normalize(stripProxyCredentials(caught as ImapFlowError));
...
const proxyConnection = async (
    ...
    host: string,
    port: number,
    options?: ProxyConnectionOptions | undefined
