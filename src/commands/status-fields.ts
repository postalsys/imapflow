...
interface StatusField {
    /** The property name the item is exposed under */
    key: string;
    /** Turns the raw response token into a usable value, or false when it is not usable */
    parser: (value: unknown) => StatusFieldValue | false;
}
...
const uint32 = (value: unknown): number | false => parseUintValue(value, MAX_UINT32_DIGITS);

const STATUS_FIELDS: { [name: string]: StatusField | undefined } = {
 ...
 */
export const parseStatusList = (list: ImapAttributeList, onField: (key: string, value: StatusFieldValue) => void): void => {
    let name: string | false | undefined;
