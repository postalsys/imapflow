...
/**
 * A waiter queued by preCheck(), resolved once IDLE is actually broken
 */
interface PreCheckWaiter {
    resolve: (value: void | PromiseLike<void>) => void;
    reject: (reason?: any) => void;
}

/**
 * State of one fallback polling session
 */
interface PollingSession {
    cancelled: boolean;
    timer: NodeJS.Timeout | null;
    preCheck: (() => Promise<void>) | null;
 ...
 */
function claimIdling(connection: ImapFlow): () => void {
    let token = {};
 ...
 */
async function runIdle(connection: ImapFlow): Promise<void | false> {
    ...
    let preCheckWaitQueue: PreCheckWaiter[] = [];
    ...
    let ownPreCheck: (() => Promise<void>) | null = null;
    ...
    try {
        ...
        let connectionPreCheck = (): Promise<void> => {
            ...
            let handler = guardedPromise<void>((resolve: (value: void | PromiseLike<void>) => void, reject: (reason?: any) => void) => {
            ...
            preCheck().catch((err: ImapFlowError) => logConnectionError(connection, 'Failed to break IDLE', err));
            ...
            onPlusTag: async () => {
                ...
                if (doneRequested) {
                    try {
                        ...
                        logConnectionError(connection, 'Failed to break IDLE', err as ImapFlowError);
                    }
                }
            },
            onSend: () => {}
        ...
        if (preCheckWaitQueue.length) {
            ...
            let waiterError = restampConnectionError(err as ImapFlowError, { rejectedFrom: 'preCheckWaiter' });
            while (preCheckWaitQueue.length) {
 ...
 */
async function pollOnce(connection: ImapFlow, session: PollingSession): Promise<void> {
 ...
 */
async function runPollingFallback(connection: ImapFlow, maxIdleTime?: number | false | undefined): Promise<void> {
 ...
 */
export default async function idle(connection: ImapFlow, maxIdleTime?: number | false | undefined): Promise<void | false | undefined> {
                ...
                idleTimer = setTimeout(() => {
                    if (connection.idling) {
                        if (typeof connection.preCheck === 'function') {
                            ...
                            connection.preCheck().catch((err: ImapFlowError) => logConnectionError(connection, 'Failed to break IDLE for restart', err));
                        }
                    }
                }, maxIdleTime);
