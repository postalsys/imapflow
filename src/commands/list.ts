...
import type { ListOptions, ListResponse, StatusObject } from '../types.js';

/**
 * How a special-use flag was determined. `name-guess` is an internal precedence tier that is
 * reported to consumers as `name`
 */
type SpecialUseSource = 'user' | SpecialUseFlagSource;

/**
 * A mailbox entry while it is being assembled from the LIST/LSUB responses. The parent path
 * fields are filled in right after the entry is created, so they are optional only until then
 */
interface ListEntry extends Omit<ListResponse, 'name' | 'parent' | 'parentPath' | 'delimiter' | 'listed' | 'subscribed'> {
    ...
    status?: (StatusObject & { error?: ImapFlowError | undefined }) | undefined;
}

/**
 * One retry stage of the main listing: which RETURN option groups to request
 */
interface ListStage {
    status: boolean;
    subscribed: boolean;
...
export default async function list(
    ...
    const PUBLIC_SOURCE: { [source: string]: 'name' } = { 'name-guess': 'name' };
    const isNameSource = (source: string | undefined) => source === 'name' || source === 'name-guess';
    ...
    try {
        ...
        let statusMap: Map<string, StatusObject> = new Map();
        let specialUseMatches: { [type: string]: Array<{ entry: ListEntry; source: SpecialUseSource }> } = {};
        ...
        let stageHasAuxArgs = (stage: ListStage) =>
            (stage.status || stage.subscribed) && stage.aux !== false && !connection.skipListAuxArgs && auxArgsAvailable;

        // Builds the RETURN (...) argument list for one retry stage
        let buildListArgs = (stage: ListStage): ImapCompileNode[] => {
            let args: ImapCompileNode[] = [];
            if (stage.status) {
        ...
        // the best match wins.
        let addSpecialUseMatch = (entry: ListEntry, type: string, source: SpecialUseSource) => {
        ...
        let normalizeFlags = (entry: ListEntry) => {
        ...
        let specialUseHints: { [path: string]: string } = Object.create(null);
        if (options.specialUseHints && typeof options.specialUseHints === 'object') {
            for (let type of Object.keys(options.specialUseHints) as Array<keyof typeof options.specialUseHints>) {
                if (
        ...
        let runList = async (reference: string, mailbox: string, returnArgs: ImapCompileNode[]) => {
            const cmdArgs: ImapCompileNode[] = [encodePath(connection, reference), encodePath(connection, mailbox)];
            ...
            let response = await connection.exec(listCommand, cmdArgs, {
                untagged: {
                    ...
                    [listCommand]: async (untagged: ImapResponse) => {
                        ...
                        if (specialUseFlag && (!isNameSource(flagSource) || !entry.flags.has('\\NonExistent'))) {
                            addSpecialUseMatch(entry, specialUseFlag, flagSource as SpecialUseSource);
                        }
                    ...
                    STATUS: async (untagged: ImapResponse) => {
                        let statusPath = normalizePath(
                            connection,
                            decodePath(connection, ((untagged.attributes![0] && untagged.attributes![0].value) || '') as string)
                        );
                        ...
                        let map: StatusObject = { path: statusPath };
                        parseStatusList(statusList, (key: string, value: unknown) => {
        ...
        let isRejectedCommand = (err: ImapFlowError) => err.responseStatus === 'BAD' && err.code !== 'ETHROTTLE';

        // Stage of the successful attempt - reused by the INBOX fixup and the LSUB
        // decision below
        let successStage: ListStage | null = null;
        ...
        for (let i = 0; i < stages.length; i++) {
            ...
            try {
                ...
                if (i === stages.length - 1 || !isRejectedCommand(err as ImapFlowError)) {
                    throw err;
                }
        ...
        if (options.listOnly) {
            return entries as ListResponse[];
        }
        ...
        if (normalizedReference && !specialUseMatches['\\Inbox']) {
            ...
            try {
                ...
                // status data.
                if (!returnArgs.length || !isRejectedCommand(err as ImapFlowError)) {
                    throw err;
                }
                ...
                if (!entry.flags.has('\\Noselect') && !entry.flags.has('\\NonExistent')) {
                    if (statusMap.has(entry.path)) {
                        ...
                        try {
                            ...
                            entry.status = { error: err } as ListEntry['status'];
                        }
                    }
                }
            }
        ...
        if (needsLsub && !connection.skipLsub) {
            try {
                ...
                if (isRejectedCommand(err as ImapFlowError)) {
                ...
                } else if ((err as ImapFlowError).responseStatus !== 'NO' || (err as ImapFlowError).code === 'ETHROTTLE') {
        ...
        return (entries as ListResponse[]).sort((a, b) => {
            ...
            let aList = ([] as string[]).concat(a.parent).concat(a.name);
