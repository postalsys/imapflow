 ...
 * QRESYNC preconditions are validated
 */
interface SelectedMailbox extends Partial<MailboxObject> {
    path: string;
    qresync?: boolean | undefined;
}

interface ValuedResponseCode {
    key: keyof SelectedMailbox;
    parse: (value: string | string[] | undefined) => bigint | number | string | Set<string> | false | undefined;
}
...
const VALUED_RESPONSE_CODES: { [code: string]: ValuedResponseCode } = Object.assign(Object.create(null), {
    ...
    highestmodseq: { key: 'highestModseq', parse: (value: unknown) => parseBigIntValue(value) },
    ...
    uidvalidity: { key: 'uidValidity', parse: (value: unknown) => parseBigIntValue(value) },
    ...
    uidnext: { key: 'uidNext', parse: (value: unknown) => parseUintValue(value, MAX_UINT32_DIGITS) },
    ...
    unseen: { key: 'unseen', parse: (value: unknown) => parseUintValue(value, MAX_UINT32_DIGITS) },
    ...
    appendlimit: { key: 'appendlimit', parse: (value: unknown) => parseUintValue(value) },
    ...
    mailboxid: {
        key: 'mailboxId',
        parse: (value: unknown) => (Array.isArray(value) ? value.length > 0 && value[0] : typeof value === 'string' && value)
    },
    ...
    permanentflags: { key: 'permanentFlags', parse: (value: unknown) => Array.isArray(value) && new Set<string>(value) }
});
...
export default async function select(
    ...
    let path = normalizePath(connection, pathInput);
    ...
    if (!connection.folders.has(path)) {
        let folders: ListResponse[] | false = await connection.run('LIST', '', path);
        if (!folders) {
    ...
    let response;
    try {
        let map: SelectedMailbox = { path };
        if (folderListData) {
            (['delimiter', 'specialUse', 'subscribed', 'listed'] as const).forEach(key => {
                if (folderListData[key]) {
                    (map as any)[key] = folderListData[key];
                }
            });
        }
        ...
        let selectCommand = {
            ...
            arguments: ([{ type: encodedPath.indexOf('&') >= 0 ? 'STRING' : 'ATOM', value: encodedPath }] as ImapCompileNode[]).concat(extraArgs || [])
        };
        ...
        response = await connection.exec(selectCommand.command, selectCommand.arguments, {
            untagged: {
                ...
                OK: async (untagged: ImapResponse) => {
                    ...
                    let section = !(untagged.attributes[0] as ImapAttributeNode).value && (untagged.attributes[0] as ImapAttributeNode).section;
                    ...
                    if (
                        ...
                        section[0] &&
                        section[0].type === 'ATOM' &&
                        (section[0].value as string | undefined)?.toUpperCase() === 'NOMODSEQ'
                    ) {
                        map.noModseq = true;
                    }
                },
                ...
                FLAGS: async (untagged: ImapResponse) => {
                ...
                EXISTS: async (untagged: ImapResponse) => {
                ...
                VANISHED: async (untagged: ImapResponse) => {
                    await connection.untaggedVanished(untagged, { path, uidNext: false, uidValidity: false } as unknown as MailboxObject);
                },
                ...
                FETCH: async (untagged: ImapResponse) => {
                    await connection.untaggedFetch(untagged, { path, uidNext: false, uidValidity: false } as unknown as MailboxObject);
                }
            }
        });
        ...
        let okAttributes: ImapAttributeList = (response.response && response.response.attributes) || [];
