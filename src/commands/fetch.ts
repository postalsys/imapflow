...
export default async function fetch(
    ...
    while (retryCount < maxRetries) {
        ...
        try {
            ...
            let attributes: ImapCompileNode[] = [{ type: 'SEQUENCE', value: (range || '*').toString() }];

            let queryStructure: ImapCompileNode[] = [];
            ...
            let setBodyPeek = (attributes: ImapCompileNode | ImapCompileNode[] | null, partial?: number[] | undefined): void => {
                let section = ([] as ImapCompileNode[]).concat(attributes || []);
            ...
            let queryAttribute: ImapCompileNode = queryStructure;
            ...
            if (options.changedSince && connection.enabled.has('CONDSTORE') && !mailbox.noModseq) {
                let changedSinceArgs: ImapCompileNode[] = [
                    {
                        type: 'ATOM',
                        value: 'CHANGEDSINCE'
                    },
