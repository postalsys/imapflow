...
const isValidSequenceSet = (value: string): boolean => value === '$' || value.split(',').every(part => SEQ_RANGE.test(part));
...
// What emitEntry() accepts: a token component, or a value that is not a string, number or
// Buffer (which yields an empty Buffer or null)
type EmitEntry = string | number | Buffer | null | undefined | false;

interface EmitOptions {
    returnEmpty?: boolean | undefined;
...
async function compiler(response: ImapCompileInput, options?: CompilerOptions | undefined): Promise<Buffer | Buffer[]> {
    ...
    let walk = async (node: ImapCompileNode, options?: { subArray?: boolean | undefined } | undefined): Promise<void> => {
        ...
        if (node && (node as { buffer?: unknown }).buffer && !Buffer.isBuffer(node)) {
            // mongodb binary
            node = (node as unknown as { buffer: ImapCompileNode }).buffer;
        }
