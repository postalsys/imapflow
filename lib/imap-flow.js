...
const { comparePaths, updateCapabilities, getFolderTree, formatMessageResponse, getDecoder, packMessageRange, normalizePath } = require('./tools');
...
class ImapFlow extends EventEmitter {
        ...
        this.clientInfo = Object.assign(
            {
                ...
                vendor: 'IMAP API',
                'support-url': 'https://github.com/andris9/imapflow/issues'
            },
    ...
    async send(data) {
        ...
        this.log.debug({ src: 's', msg: logCompiled.toString(), cid: this.id });
    ...
    async reader() {
        ...
        while ((data = this.streamer.read()) !== null) {
            ...
            if (/^\d+$/.test(parsed.command) && parsed.attributes && parsed.attributes[0] && parsed.attributes[0].value === 'FETCH') {
                ...
                this.log.trace({ src: 's', msg: logCompiled.toString(), cid: this.id });
            } else {
                this.log.debug({ src: 's', msg: logCompiled.toString(), cid: this.id });
            }
    ...
    setSocketHandlers() {
        ...
        this._socketClose =
            ...
            (() => {
                this.close();
                // Notify pending command handlers
                for (let pendingCommand of this.requestTagMap.values()) {
    ...
    async startSession() {
        ...
        if (nsResponse.error && nsResponse.status === 'BAD' && /User is authenticated but not connected/i.test(nsResponse.text)) {
    ...
    async download(range, part, options) {
        ...
        let getNextPart = async query => {
            ...
            if (query.bodyParts) {
                ...
                if (part === '1' && !result.mime) {
                    // Outlook does not support requesting 1.MIME for text/* messages, so we issue an additional command
                    let mimeResponse = await this.fetchOne(range, { bodyParts: ['header'] }, options);
                    if (!mimeResponse) {
                        return { response: false, chunk: false };
                    }
                    result.mime = mimeResponse.headers;
                }
            }
    ...
    async processLocks(force) {
        ...
        const release = () => {
            if (this.currentLockId) {
                this.log.trace({ msg: 'Mailbox lock released', lockId: this.currentLockId });
                this.currentLockId = 0;
            }
        ...
        for (let level of levels) {
            synteticLogger[level] = (...args) => {
                ...
                if (this.emitLogs && args && args[0] && typeof args[0] === 'object') {
                    let logEntry = Object.assign({ level, t: Date.now(), cid: this.cid, lo: ++this.lo }, args[0]);
