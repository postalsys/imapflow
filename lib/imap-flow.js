...
class ImapFlow extends EventEmitter {
    ...
    async reader() {
        ...
        while ((data = this.streamer.read()) !== null) {
            ...
            try {
                ...
                this.log.error({ src: 's', msg: data.payload.toString(), error: err.message, cid: this.id });
                data.next();
                continue;
            }
            ...
            if (parsed.tag === '*' && parsed.command) {
                ...
                if (untaggedHandler) {
                    try {
                        ...
                        this.log.error({ error: err, cid: this.id });
                        data.next();
                        continue;
                    }
                }
    ...
    setEventHandlers() {
        this.socketReadable = () => {
            if (!this.reading) {
                this.reading = true;
                this.reader()
                    .catch(err => this.log.error({ error: err, cid: this.id }))
                    .finally(() => {
                        this.reading = false;
                    });
            }
    ...
    autoidle() {
        ...
        this.idleStartTimer = setTimeout(() => {
            this.idle().catch(err => this.log.error({ error: err, cid: this.id }));
        }, 15 * 1000);
    }
    ...
    close() {
        ...
        if (typeof this.preCheck === 'function') {
            this.preCheck().catch(err => this.log.error({ error: err, cid: this.id }));
        }
