...
class ImapFlow extends EventEmitter {
    ...
    async send(data) {
        ...
        this.log.info({ src: 's', msg: logCompiled.toString(), cid: this.id });
    ...
    async reader() {
        ...
        while ((data = this.streamer.read()) !== null) {
            ...
            if (parsed.tag === '+' && this.commandParts.length) {
                ...
                this.write(content);
                this.log.info({ src: 'c', msg: `(* ${content.length}B continuation *)`, cid: this.id });
                data.next();
                continue;
            }
    ...
    async untaggedFetch(untagged) {
        ...
        // Filter duplicate responses if possible.
        // Unless SILENT is used then some server may trigger untagged FETCH for
        // flags change 2 times - one as a response for STORE and another as flag update notification
        if (message.modseq) {
            let lastSeenModseq = this.modseqMap.get(this.mailbox);
            if (lastSeenModseq && lastSeenModseq >= message.modseq) {
                // already seen
                return;
            }
            this.modseqMap.set(this.mailbox, message.modseq);
        }
    ...
    async processLocks(force) {
        ...
        const { resolve, reject, path, options } = this.locks.shift();
    ...
    async getMailboxLock(path, options) {
        ...
        return await new Promise((resolve, reject) => {
            this.locks.push({ resolve, reject, path, options });
