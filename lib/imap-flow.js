 ...
 * @property {Set} flags - list of flags for this mailbox
 ...
 * @property {Set} permanentFlags - A Set of flags available to use in this mailbox. If it is not set or includes special flag "\\\*" then any flag can be used.
 ...
 * @property {Set} flags - A set of all flags for the updated message
 */
...
class ImapFlow extends EventEmitter {
    constructor(options) {
        ...
        this.log = logger.child({ component: 'imap-connection', cid: this.id });
     ...
     * @property {string} delimiter - mailbox path delimiter, usually "." or "/"
     * @property {Set} flags - a set of flags for this mailbox
     * @property {string} specialUse - one of special-use flags (if applicable): "\All", "\Archive", "\Drafts", "\Flagged", "\Junk", "\Sent", "\Trash". Additionally INBOX has non-standard "\Inbox" flag set
     * @property {boolean} listed - *true* if mailbox was found from the output of LIST command
     * @property {boolean} subscribed - *true* if mailbox was found from the output of LSUB command
     */
     ...
     * @property {Date|DateString} [before] - Matches messages received before date
     * @property {Date|DateString} [on] - Matches messages received on date (ignores time)
     * @property {Date|DateString} [since] - Matches messages received after date
     * @property {Date|DateString} [sentBefore] - Matches messages sent before date
     * @property {Date|DateString} [sentOn] - Matches messages sent on date (ignores time)
     * @property {Date|DateString} [sentSince] - Matches messages sent after date
     ...
     * @param {Date|DateString} [idate=now] - internal date to be set for the message
     ...
     * @property {Map} [uidMap] - Map of UID values (if server has UIDPLUS extension enabled) where key is UID in source mailbox and value is the UID for the same message in destination mailbox
     */
     ...
     * @property {arMessageAddressObject[]} [to] - Array of addresses from the To: header
     ...
     * @property {Set} [labels] - a Set of labels. Only present if server supports X-GM-EXT-1 extension
     * @property {number} [size] - message size
     * @property {Set} [flags] - a set of message flags
     ...
     * @property {Map} [bodyParts] - a Map of message body parts where key is requested part identifier and value is a Buffer or Headers object (all x.header and x.mime keys)
     ...
     * let mailbox = await client.mailboxOpen('INBOX');
     * // fetch UID for all messages in a mailbox
     * let lastMsg = client.fetchOne('*', {uid: true})
     * console.log(lastMsg.uid);
     */
