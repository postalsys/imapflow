...
const normalizeAutoIdleDelay = (value, socketTimeout, log, cid) => {
    const maxDelay = Math.max(0, socketTimeout - AUTO_IDLE_SOCKET_MARGIN);
    ...
    if (delay > maxDelay) {
        reason = `above socketTimeout (${socketTimeout} ms)`;
        delay = maxDelay;
    }
...
class ImapFlow extends EventEmitter {
    ...
    setSocketHandlers() {
         ...
         * by sending a NOOP command and then returning to IDLE state. An open download stream is
         * treated the same way: a consumer that stops draining leaves the socket quiet for as long
         * as it stalls, and that is a slow reader rather than a dead connection.
         *
        ...
        this._socketTimeout =
            ...
            (() => {
                ...
                if (this.idling || this._openDownloads) {
    ...
    autoidle() {
        ...
        // The single definition of "the connection is free". A held or queued mailbox lock, a
        ...
        // declining here postpones IDLE, it never cancels it.
        if (this.currentLock || this.locks.length || this.currentRequest || this.requestQueue.length || this._openDownloads) {
            return;
        }
    ...
    close() {
        try {
            ...
            this.currentLock = false;
            // Reset with the rest of the connection ownership state. Unlike currentLock and
            // currentRequest this is bookkeeping rather than state the connection maintains for
            // its own reasons, so a decrement missed on some future path would otherwise leave
            // auto-IDLE off for good with nothing observable.
            this._openDownloads = 0;
    ...
    async download(range, part, options) {
        ...
        let runFetchAllParts = () => {
            ...
            // trips per chunk, for as long as the consumer is slow.
            this._openDownloads++;
            fetchAllParts()
                .catch(err => {
    ...
    async run(command, ...args) {
        ...
        try {
            ...
            if (command !== 'IDLE') {
                ...
                // Re-armed whether or not the command succeeded: the clearTimeout above is
                // unconditional, so skipping the failure path would leave auto-IDLE disarmed on an
                // otherwise healthy connection until some later command happened to succeed.
                this.autoidle();
            }
        }
    }
        ...
        try {
            ...
            while (this.locks.length > 0) {
                ...
                if (this.mailbox && this.mailbox.path === path && !!this.mailbox.readOnly === !!options.readOnly) {
                    ...
                    armHeldTimer();
                    // Acquiring a lock can itself run a command (the SELECT behind mailboxOpen),
                    // and run() re-arms auto-IDLE when that command settles - a moment before
                    // currentLock is set. Re-checking here clears that timer again.
                    this.autoidle();
