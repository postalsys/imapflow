...
class ImapFlow extends EventEmitter {
     ...
     * @param {Boolean} [options.tls.disableSTARTTLS=false] Do not use STARTTLS, even if the server advertizes it in IMAP `CAPABILITY`. This helps with servers that have a broken TLS configuration. If this is `true` and `options.secure == false`, then a plain unencrypted socket is used. Be sure to clearly warn the user about the consequences.
