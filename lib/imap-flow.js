...
class ImapFlow extends EventEmitter {
    ...
    exec(command, attributes, options) {
        if (this.state === this.states.LOGOUT || this.isClosed) {
            let p = Promise.reject(this.createNoConnectionError(false, { rejectedFrom: 'execClosed', command }));
            p.catch(noop);
            return p;
        }
        ...
        if (!this.socket || this.socket.destroyed) {
            ...
            error.cid = this.id;
            error.rejectedFrom = 'execNoSocket';
    ...
    //
    // The connection id always travels on the error, and each call site names itself through
    // `meta` (`rejectedFrom`, plus the command or mailbox path it belongs to). A stack trace only
    // records where an error was built, and close() hands a rejection to every pending request and
    // lock in the same tick, so without these an error that reaches a global unhandledRejection
    // handler arrives with nothing that identifies the connection it came from, let alone which of
    // the rejected promises carried it.
    ...
    close() {
        try {
            ...
            const createNoConnectionError = (byeReason, meta) => this.createNoConnectionError(byeReason, meta);
            ...
            for (let request of pendingRequests) {
                request.reject(createNoConnectionError(byeReason, { rejectedFrom: 'pendingRequest', command: request.command }));
            }
            ...
            if (this.locks && this.locks.length) {
                ...
                for (let lock of pendingLocks) {
                    ...
                    if (typeof lock.reject === 'function') {
                        lock.reject(createNoConnectionError(byeReason, { rejectedFrom: 'mailboxLock', path: lock.path }));
                    }
                }
            }
