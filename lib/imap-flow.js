...
class ImapFlow extends EventEmitter {
     ...
     * This uses the SORT extension (RFC 5256) if available.
     *
     * @param {Object|Array} sortCriteria Sort criteria. Can be an array like `['REVERSE', 'DATE']`
     *   or `[{reverse: true}, 'date']`. Valid sort keys: ARRIVAL, CC, DATE, FROM, SIZE, SUBJECT, TO.
     * @param {SearchObject} query Query to filter the messages
     * @param {Object} [options]
     * @param {Boolean} [options.uid] If `true` then returns UID numbers instead of sequence numbers
     * @returns {Promise<Number[]|false>} An array of sequence or UID numbers sorted by the criteria,
     *   or `false` if SORT is not supported by the server
     *
     * @example
     * let mailbox = await client.mailboxOpen('INBOX');
     * // Check if SORT is supported
     * if (client.capabilities.has('SORT')) {
     *   // find all unseen messages, sorted by date (newest first)
     *   let list = await client.sort([{reverse: true}, 'date'], {seen: false}, {uid: true});
     * } else {
     *   // fallback to regular search + client-side sort
     *   let list = await client.search({seen: false}, {uid: true});
     * }
     */
    async sort(sortCriteria, query, options) {
        if (!this.mailbox) {
            // no mailbox selected, nothing to do
            return;
        }

        let response = await this.run('SORT', sortCriteria, query, options);
