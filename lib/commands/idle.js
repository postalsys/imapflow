...
async function runIdle(connection) {
    ...
    try {
        ...
        let connectionPreCheck = () => {
            ...
            // Same guard exec(), connect() and getMailboxLock() carry, and for the same reason:
            // the catch block below rejects every queued waiter synchronously while close() tears
            // the connection down, so the rejection must be observed even if the caller's own
            // handler is not attached by the time the microtask queue drains. The rejection still
            // propagates normally to whoever awaits the returned promise.
            handler.catch(noop);
