...
module.exports = {
    normalizePath(connection, path) {
        ...
        if (!connection.enabled.has('UTF8=ACCEPT') && /[\x80-\xff]/.test(path)) {
            try {
        ...
        for (let i = 0, len = attributes.length; i < len; i++) {
            ...
            switch (key) {
                ...
                case 'x-gm-labels':
                    ...
                    // make sure that current path is also included, except \All which is a virtual folder
                    if (connection.mailbox.specialUse && connection.mailbox.specialUse !== '\\All') {
                        map.labels.add(connection.mailbox.specialUse);
                    } else if (!connection.mailbox.specialUse) {
                        map.labels.add(connection.mailbox.path);
                    }
                    */
                ...
                default:
                    if (key.indexOf('body[') >= 0) {
                        let partKey = key.replace(/^body\[|]$/g, '');
    ...
    parseEnvelope(entry) {
        let processAddresses = function(list) {
                return [].concat(list || []).map(function(addr) {
                    return {
                        name: libmime.decodeWords((addr[0] && addr[0].value) || ''),
        ...
        if (entry[0] && entry[0].value) {
            let date = new Date(entry[0].value);
            if (date.toString() === 'Invalid Date') {
                envelope.date = entry[0].value;
            } else {
                envelope.date = date;
            }
        }
        ...
        if (entry[1] && entry[1].value) {
            envelope.subject = libmime.decodeWords(entry[1] && entry[1].value);
        }
        ...
        if (entry[8] && entry[8].value) {
            envelope.inReplyTo = entry[8].value;
        }

        if (entry[9] && entry[9].value) {
            envelope.messageId = entry[9].value;
        }
