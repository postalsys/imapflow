...
describe('commands-branches', () => {
    ...
    it('Branches: copy with undefined options uses {} fallback', async () => {
        ...
        assert.ok(result);
        assert.equal(execCmd, 'COPY'); // not UID COPY, since options.uid is undefined
    });
    ...
    // ============================================================================
    it('Branches: status with undefined query uses {} fallback', async () => {
        ...
        const result = await (statusCommand as any)(connection, 'INBOX');
    ...
    it('Branches: append tagged section with non-string code uses empty fallback', async () => {
        ...
        assert.ok(result);
        assert.equal(result.destination, 'Sent');
    ...
    it('Branches: quota QUOTAROOT without root name yields false', async () => {
        ...
        assert.equal(result.quotaRoot, undefined);
    });
        ...
        const connection: any = createMockConnection({
            ...
            exec: async () => {
                ...
                if (calls === 1) {
                    ...
                    err.code = 'ETHROTTLE';
                    (err as any).throttleReset = 1001; // > backoffDelay(1000) -> throttleReset branch taken
    ...
    it('Branches: search ESEARCH without uid emits SEARCH and skips the tag correlator', async () => {
        ...
        const result: any = await searchCommand(connection, true, { returnOptions: ['COUNT'] });
        assert.equal(execCmd, 'SEARCH'); // not 'UID SEARCH'
    ...
    // ============================================================================
    it('Branches: parseEsearchResponse PARTIAL with non-list value is ignored', () => {
        const parse = parseEsearchResponse;
    ...
    it('Branches: select filters non-string flags in FLAGS and PERMANENTFLAGS', async () => {
        ...
        assert.equal(result.flags!.size, 1);
        // Non-string permanent flag filtered out
        assert.ok(result.permanentFlags!.has('\\*'));
        assert.equal(result.permanentFlags!.size, 1);
    });
    ...
    it('Branches: select FLAGS guard ignores malformed attributes without throwing', async () => {
        const makeConn = (flagsAttributes: any) =>
        ...
        const emptyResult = await selectCommand(makeConn([]) as any, 'INBOX');
        assert.ok(emptyResult);
        assert.equal(emptyResult.flags, undefined);

        // attributes[0] present but not an array: bail on the Array.isArray guard.
        const nonArrayResult = await selectCommand(makeConn([{ value: '\\Seen' }]) as any, 'INBOX');
        assert.ok(nonArrayResult);
        assert.equal(nonArrayResult.flags, undefined);
    });
    ...
    it('Branches: list with undefined mailbox pattern uses empty fallback', async () => {
        ...
        assert.ok(Array.isArray(result));
        assert.equal(listMailboxArg, '');
        assert.equal(lsubMailboxArg, '');
    });
    ...
    // ============================================================================
    it('Branches: idle NOOP fallback runs poll then breaks via preCheck', async () => {
    ...
    it('Branches: idle IDLE path with currentLock drives preCheck and cleanup', async () => {
        ...
        const connection: any = createMockConnection({
            ...
            exec: (cmd: any, attrs: any, opts: any) => {
        ...
        const breakPromise = typeof (connection as any).preCheck === 'function' ? (connection as any).preCheck() : Promise.resolve();
    ...
    // ============================================================================
    it('Branches: idle cleanup drains queued waiters when DONE never sent', async () => {
        let onPlusTagFn = null;
        let resolveExec: any = null;
        let resolveEntered: any;
        const execEntered = new Promise(resolve => {
            resolveEntered = resolve;
        });
        ...
        const breakPromise = (connection as any).preCheck();
        ...
        assert.ok(typeof onPlusTagFn === 'function');
        resolveExec();
    ...
    // ============================================================================
    it('Branches: idle onPlusTag catches preCheck error when DONE write throws', async () => {
        let onPlusTagFn: any = null;
        let resolveExec: any = null;
        let resolveEntered: any;
        const execEntered = new Promise(resolve => {
            resolveEntered = resolve;
