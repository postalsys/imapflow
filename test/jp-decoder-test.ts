import { describe, it } from 'node:test';
import assert from 'node:assert/strict';
import { JPDecoder } from '../src/jp-decoder.js';
import { PassThrough, Transform } from 'node:stream';
import encodingJapanese from 'encoding-japanese';

// Helper to collect stream output
const collectStream: any = (stream: any) =>
    new Promise((resolve, reject) => {
        const chunks: any = [];
        stream.on('data', (chunk: any) => chunks.push(chunk));
        stream.on('end', () => resolve(Buffer.concat(chunks)));
        stream.on('error', reject);
    });

describe('jp-decoder', () => {
    // ============================================
    // Constructor tests
    // ============================================
    it('JPDecoder: constructor sets charset', () => {
        let decoder = new JPDecoder('iso-2022-jp');
        assert.equal(decoder.charset, 'iso-2022-jp');
        assert.deepEqual(decoder.chunks, []);
        assert.equal(decoder.chunklen, 0);
    });
    it('JPDecoder: is a Transform stream', () => {
        let decoder = new JPDecoder('iso-2022-jp');
        assert.ok(decoder instanceof Transform);
        assert.ok(typeof decoder.pipe === 'function');
        assert.ok(typeof decoder.write === 'function');
    });

    // ============================================
    // _transform tests
    // ============================================
    it('JPDecoder: _transform accumulates buffer chunks', (t, done) => {
        let decoder = new JPDecoder('iso-2022-jp');
        let chunk1 = Buffer.from('hello');
        let chunk2 = Buffer.from('world');

        decoder._transform(chunk1, 'buffer' as any, () => {
            assert.equal(decoder.chunks.length, 1);
            assert.equal(decoder.chunklen, 5);

            decoder._transform(chunk2, 'buffer' as any, () => {
                assert.equal(decoder.chunks.length, 2);
                assert.equal(decoder.chunklen, 10);
                done();
            });
        });
    });
    it('JPDecoder: _transform converts string to buffer', (t, done) => {
        let decoder = new JPDecoder('iso-2022-jp');
        let stringChunk = 'hello';

        decoder._transform(stringChunk, 'utf8', () => {
            assert.equal(decoder.chunks.length, 1);
            assert.ok(Buffer.isBuffer(decoder.chunks[0]));
            assert.equal(decoder.chunks[0].toString(), 'hello');
            done();
        });
    });

    // ============================================
    // _flush tests
    // ============================================
    it('JPDecoder: _flush outputs accumulated data', async () => {
        let decoder = new JPDecoder('utf-8');
        let output = collectStream(decoder);

        decoder.write(Buffer.from('hello '));
        decoder.write(Buffer.from('world'));
        decoder.end();

        let result: any = await output;
        assert.equal(result.toString(), 'hello world');
    });
    it('JPDecoder: _flush converts ISO-2022-JP to Unicode', async () => {
        let decoder = new JPDecoder('iso-2022-jp');
        let output = collectStream(decoder);

        // ISO-2022-JP encoded Japanese text for "nihongo" (Japanese)
        // ESC $ B sequence switches to JIS X 0208, ESC ( B switches back to ASCII
        let iso2022jp = Buffer.from([
            0x1b,
            0x24,
            0x42, // ESC $ B - switch to JIS X 0208
            0x46,
            0x7c, // ni
            0x4b,
            0x5c, // hon
            0x38,
            0x6c, // go
            0x1b,
            0x28,
            0x42 // ESC ( B - switch back to ASCII
        ]);

        decoder.write(iso2022jp);
        decoder.end();

        let result: any = await output;
        assert.ok(result.length > 0);
    });
    it('JPDecoder: _flush handles conversion errors gracefully', async () => {
        // Use an invalid/unknown charset to trigger error path
        let decoder = new JPDecoder('invalid-charset-xyz');
        let output = collectStream(decoder);

        let input = Buffer.from('test data');
        decoder.write(input);
        decoder.end();

        let result: any = await output;
        // On error, should return original input
        assert.equal(result.toString(), 'test data');
    });
    it('JPDecoder: _flush falls back to raw input when convert throws', async () => {
        // encoding-japanese rarely throws on its own, so force it: the module object
        // is shared by reference with the copy held inside jp-decoder.js, so patching
        // .convert here affects the decoder's call too.
        let originalConvert = encodingJapanese.convert;
        encodingJapanese.convert = () => {
            throw new Error('forced conversion failure');
        };

        try {
            let decoder = new JPDecoder('iso-2022-jp');
            let output = collectStream(decoder);

            let input = Buffer.from('raw bytes preserved');
            decoder.write(input);
            decoder.end();

            let result: any = await output;
            // On a thrown error the catch block pushes the original, untouched input
            assert.equal(result.toString(), 'raw bytes preserved');
        } finally {
            encodingJapanese.convert = originalConvert;
        }
    });
    it('JPDecoder: _flush converts non-string convert output to buffer', async () => {
        // When convert returns a non-string (e.g. an array of code units), it must be
        // pushed as-is without the Buffer.from(string) branch.
        let originalConvert = encodingJapanese.convert;
        encodingJapanese.convert = () => Buffer.from('converted buffer');

        try {
            let decoder = new JPDecoder('iso-2022-jp');
            let output = collectStream(decoder);

            decoder.write(Buffer.from('whatever'));
            decoder.end();

            let result: any = await output;
            assert.equal(result.toString(), 'converted buffer');
        } finally {
            encodingJapanese.convert = originalConvert;
        }
    });
    it('JPDecoder: _flush handles empty input', async () => {
        let decoder = new JPDecoder('iso-2022-jp');
        let output = collectStream(decoder);

        decoder.end();

        let result: any = await output;
        assert.equal(result.length, 0);
    });

    // ============================================
    // Integration tests
    // ============================================
    it('JPDecoder: works with pipe', async () => {
        let source = new PassThrough();
        let decoder = new JPDecoder('utf-8');
        let output = collectStream(source.pipe(decoder));

        source.write('hello ');
        source.write('world');
        source.end();

        let result: any = await output;
        assert.equal(result.toString(), 'hello world');
    });
    it('JPDecoder: handles multiple small chunks', async () => {
        let decoder = new JPDecoder('utf-8');
        let output = collectStream(decoder);

        // Write character by character
        'hello'.split('').forEach(char => decoder.write(char));
        decoder.end();

        let result: any = await output;
        assert.equal(result.toString(), 'hello');
    });
    it('JPDecoder: handles Shift_JIS charset', async () => {
        let decoder = new JPDecoder('shift_jis');
        let output = collectStream(decoder);

        // Shift_JIS encoded "test" in katakana (tesuto)
        let shiftJis = Buffer.from([0x83, 0x65, 0x83, 0x58, 0x83, 0x67]);

        decoder.write(shiftJis);
        decoder.end();

        let result: any = await output;
        assert.ok(result.length > 0);
    });
    it('JPDecoder: handles EUC-JP charset', async () => {
        let decoder = new JPDecoder('euc-jp');
        let output = collectStream(decoder);

        // EUC-JP encoded Japanese character
        let eucJp = Buffer.from([0xc6, 0xfc, 0xcb, 0xdc]); // nihon

        decoder.write(eucJp);
        decoder.end();

        let result: any = await output;
        assert.ok(result.length > 0);
    });

    // ============================================
    // maxBytes buffering bound
    // ============================================
    it('JPDecoder: buffers at most maxBytes', () => {
        // The whole-input buffering must honor the caller's download limit: without the
        // bound a server could defeat maxBytes just by labelling a part with a Japanese
        // charset, since nothing is emitted (and limited) until _flush
        let decoder = new JPDecoder('iso-2022-jp', 8);
        decoder.resume();

        decoder.write(Buffer.from('hello '));
        decoder.write(Buffer.from('world'));

        assert.equal(decoder.chunklen, 8);
        assert.equal(Buffer.concat(decoder.chunks).toString(), 'hello wo');
        decoder.end();
    });
    it('JPDecoder: a fractional maxBytes still bounds the decoder', () => {
        // A computed limit (a percentage of a budget, say) is not an integer, and silently turning
        // that into "no limit at all" is exactly the exposure the bound exists to close
        const decoder = new JPDecoder('sjis', 512.5);
        assert.equal(decoder.maxBytes, 512, 'a fractional bound is floored to a whole byte count');
        decoder.resume();
        decoder.write(Buffer.alloc(1024, 0x61));
        assert.equal(decoder.chunklen, 512, 'nothing beyond the bound is buffered');
        assert.equal(decoder.limited, true, 'the decoder reports that it is full');
        decoder.end();
    });
    it('JPDecoder: a non-numeric maxBytes leaves the decoder unbounded', () => {
        assert.equal(new JPDecoder('sjis').maxBytes, Infinity);
        assert.equal(new JPDecoder('sjis', Infinity).maxBytes, Infinity);
        assert.equal(new JPDecoder('sjis', 'lots' as any).maxBytes, Infinity);
        assert.equal(new JPDecoder('sjis', -1).maxBytes, Infinity);
        assert.equal(new JPDecoder('sjis').limited, false);
    });
    it('JPDecoder: reports limited once the bound is reached', () => {
        // The decoder emits nothing until _flush, so a limiter downstream cannot tell that it is
        // full - the download loop reads this flag instead to stop pulling from the server
        const decoder = new JPDecoder('sjis', 10);
        decoder.resume();
        assert.equal(decoder.limited, false);
        decoder.write(Buffer.alloc(4, 0x61));
        assert.equal(decoder.limited, false, 'not full yet');
        decoder.write(Buffer.alloc(6, 0x61));
        assert.equal(decoder.limited, true);
        assert.equal(decoder.chunklen, 10);
        decoder.end();
    });
});
